                ...
                <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center justify-center">
                        <div class="flex items-center justify-center relative" style="height: 100px;">
                            ...
                            <div id="editButton"
                                class="absolute right-0 top-0 cursor-pointer bg-white border rounded-full p-1 hover:bg-lime-300">
                ...
                <form action="{{ route('clubs.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center justify-center">
                        <div class="flex items-center justify-center relative" style="height: 100px;">
                            ...
                            <input type="file" id="fileInputClub" name="photo_url" style="display: none;">
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col gap-2 items-left justify-left">
                            @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as $day)
                                <div class="flex flex-col gap-2 mt-4">
                                    ...
                                    <div class="flex items-center gap-4 text-xs ml-5">
                                        <label for="{{ $day }}_apertura">Apertura:</label>
                                        <input type="time" id="{{ $day }}_apertura"
                                            name="{{ $day }}_apertura"
                                            value="{{ optional($profile->hours->where('day_of_week', $day)->first())->opening_time ? \Carbon\Carbon::createFromFormat('H:i:s', $profile->hours->where('day_of_week', $day)->first()->opening_time)->format('H:i') : '' }}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-400 focus:border-lime-400 w-32 p-2.5">
                                        <label for="{{ $day }}_cierre">Cierre:</label>
                                        <input type="time" id="{{ $day }}_cierre"
                                            name="{{ $day }}_cierre"
                                            value="{{ optional($profile->hours->where('day_of_week', $day)->first())->closing_time ? \Carbon\Carbon::createFromFormat('H:i:s', $profile->hours->where('day_of_week', $day)->first()->closing_time)->format('H:i') : '' }}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-400 focus:border-lime-400 w-32 p-2.5 ml-2">
                                    </div>
                                </div>
                            @endforeach
                        </div>
